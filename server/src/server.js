...
function startGame(roomId, room) {
    ...
    // set random timer
    const randomDelay = Math.floor(Math.random() * (MAX_TIMER - MIN_TIMER)) + MIN_TIMER;
        ...
        if (message.type === "JOIN_ROOM") {
            ...
            if (room.players.find((p) => p.name === playerName)) {
                ws.send(JSON.stringify({
                    type: "ERROR",
                    code: "DUPLICATE_NAME",
                    message: "Player name already taken in this room. Please change it and try again.",
                }));
                return;
            }
            ...
            console.log(`👋 ${playerName} left room ${clientData.roomId} from Game scene`);
